with recursive vencimentos as (
    ...
    left join
        emp_venc
        on
            empregado.matr = emp_venc.matr
    left join
        vencimento
        on
            emp_venc.cod_venc = vencimento.cod_venc
    group by
        empregado.matr, empregado.lotacao, empregado.lotacao_div
),
...
descontos as (
    ...
    left join
        emp_desc
        on
            empregado.matr = emp_desc.matr
    left join
        desconto
        on
            emp_desc.cod_desc = desconto.cod_desc
    group by
        empregado.matr, empregado.lotacao, empregado.lotacao_div
),
...
salarios_liquidos as (
    ...
    inner join
        descontos
        on
            vencimentos.matr = descontos.matr
)
...
inner join
    departamento
    on
        divisao.cod_dep = departamento.cod_dep
inner join
    salarios_liquidos
    on
...
group by
    divisao.cod_divisao, divisao.cod_dep, departamento.nome, divisao.nome
order by
    media desc;
